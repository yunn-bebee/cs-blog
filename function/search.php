...
<?php
    //Check if the user searches
        if (isset($_GET["srch"]) and $_GET["srch"] != "" and $_GET["srch"] != " ") {
            $query = $_GET['search'];
            //Displays the search query 
            echo "<h1 class='text-center header-text p-4 text-light'>Search:<strong class='p'>$query</strong></h1>";
            ?>
     </header>
     <div class="blog container-fluid p-3 d-flex flex-wrap align-items-center justify-content-evenly">
            ...
            $sql = "SELECT blog_posts.*, user.username, tags.tag_name FROM blog_posts
                    ...
                    WHERE blog_posts.title LIKE '%$query%' 
                    OR blog_posts.content LIKE '%$query%'
            ...
            $totalRecordsQuery = "SELECT COUNT(*) AS total FROM blog_posts WHERE title LIKE '%$query%' OR content LIKE '%$query%'";
            ...
            $totalPages = ceil($totalRecords / $recordsPerPage);
                ...
                generatePaginationLinks($currentPage,$totalPages,'search.php');
